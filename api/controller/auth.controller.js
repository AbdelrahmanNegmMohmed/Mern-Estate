...
export const google = async (request,response, next)=>{
  try{
    ...
    if(user){
      ...
      const newUser = new User({ username: req.body.name.split(" ").join("").toLowerCase() + Math.random().toString(36).slice(-4) , email: request.body.email, password: hashedPassword, avatar: request.body.photo });
      await newUser.save();
